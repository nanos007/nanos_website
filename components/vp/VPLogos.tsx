...
function SubLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 mb-6">
      <span className="text-[14px] font-bold tracking-[0.2em] uppercase text-ink/50">{children}</span>
        ...
        <div className="space-y-14">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-8 items-center">
              {current.map((emp) => (
                <a key={emp.name} href={emp.url} target="_blank" rel="noopener noreferrer"
                  className="opacity-70 hover:opacity-100 transition-opacity grayscale hover:grayscale-0" aria-label={emp.name}>
          ...
          <div className="border-t border-hairline pt-10">
            ...
            <div className="flex flex-wrap gap-10 items-center">
              {previous.map((emp) => (
                <a key={emp.name} href={emp.url} target="_blank" rel="noopener noreferrer"
                  className="opacity-50 hover:opacity-80 transition-opacity grayscale hover:grayscale-0" aria-label={emp.name}>
                  <Image src={emp.logo} alt={emp.name} width={emp.w} height={emp.h} style={{ height: emp.h, width: "auto" }} className="object-contain" />
                </a>
              ))}
            </div>
          ...
          <div className="border-t border-hairline pt-10">
            ...
            <div className="space-y-10">
              <div>
                ...
                <div className="flex flex-wrap gap-8 items-center">
                  {defenseClients.map((c) => <ClientEntry key={c.name} {...c} />)}
                </div>
              </div>
              <div>
                <SubLabel>Maritime</SubLabel>
                <div className="flex flex-wrap gap-8 items-center">
                  {maritimeClients.map((c) => <ClientEntry key={c.name} {...c} />)}
                </div>
              </div>
            </div>
          </div>
